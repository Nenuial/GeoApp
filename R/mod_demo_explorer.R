...
mod_demo_explorer_server <- function(id) {
  moduleServer(id, function(input, output, session) {
        ...
        leaflet::leaflet(
          options = leaflet::leafletOptions(
            crs = leaflet::leafletCRS(
              crsClass = "L.Proj.CRS", code = "ESRI:54030",
          ...
          # Modified leaflet add_legend to add decresing parameter
          GeoApp::add_legend(
            "bottomleft",
    ...
    get_data <- memoise::memoise(get_data_fun,
      ~ format(Sys.Date(), "%Y"),
      cache = memoise::cache_filesystem(paste0(
        rappdirs::user_cache_dir("geoapp"),
        "/.Rcache"
      ))
    )
